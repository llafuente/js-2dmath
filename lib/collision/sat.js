 ...
 */
function _isSeparatingAxis(a_pos, b_pos, a_points, b_points, axis, response) {
    ...
    if (response) {
        ...
        if (absOverlap < response.penetration) {
            response.penetration = absOverlap;
            vec2_copy(response.penetrationN, axis);
            if (overlap < 0) {
                vec2_negate(response.penetrationN, response.penetrationN);
            }
        }
    }
...
function getPolygonPolygon(out_response, a_points, a_normals, a_pos, b_points, b_normals, b_pos) {
    ...
    for (i = 0; i < aLen; ++i) {
        if (_isSeparatingAxis(a_pos, b_pos, a_points, b_points, a_normals[i], out_response)) {
            return false;
        }
    }
    ...
    for (i = 0;i < bLen; ++i) {
        if (_isSeparatingAxis(a_pos, b_pos, a_points, b_points, b_normals[i], out_response)) {
            return false;
        }
    }
